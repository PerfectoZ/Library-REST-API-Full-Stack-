...
export const BookCheckoutPage = () => {
    ...
    useEffect(() => {
        const fetchBookReviews = async () => {
            ...
            if(!responseReviews.ok) {
                throw new Error('Something went wrong in fetching review!');
            }
    ...
    useEffect(() => {
        const fetchUserCurrentLoansCount = async () => {
            if(authState && authState.isAuthenticated) {
                const url = 'http://localhost:8080/books/secure/currentLoans/count';
                ...
                if(!currentLoansCountResponse.ok) {
                    throw new Error('Something went wrong in fetching current loans count!');
                }
    ...
    useEffect(() => {
        const fetchUserCheckedOutBook = async () => {
            if(authState && authState.isAuthenticated) {
                const url = `http://localhost:8080/books/secure/isCheckedOut/byUser?bookId=${bookId}`;
                const requestOptions = {
                    method: 'GET',
                    headers: {
                        Authorization: `Bearer ${authState.accessToken?.accessToken}`,
